...
def batsman():
    ...
    # batsman - faced balls into a dict
    with open("ipl-stats.csv","r") as ipl:
        csvreader=csv.DictReader(ipl)
        balls_faced={}
        for ball in csvreader:
    ...
    # strike rate of batsmans in dict
    strike_rate={}
...
def all_rounders():
    ...
    # selecting players in both best batsman and bowler list 
    best={}
    for i in bt:
        if i in bw:
            best[i] = bt[i] * bw [i]
...
squad=["CHRIS JONATHAN SELWYN ʕ•ᴥ•ʔ","DEEPAK VASAN (✿◠‿◠)","MUTHU SRIMAN :-}","HARI HARA SUDHAN (˘▾˘~)","BHANUPRATHAP (｡◕‿◕｡)","DHANUSH (ᵔᴥᵔ) "]
print("SQUAD :-")
for i in squad:
    print(" *",i)
